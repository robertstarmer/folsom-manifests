...
$company_ntp_server	= "192.168.50.1"
...
$cobbler_node_ip 	= '192.168.50.254'
$node_subnet 		= '192.168.50.0'
...
$controller_node_address       = '192.168.50.10'
$controller_node_network       = '192.168.50.0'
...
$floating_ip_range       = '192.168.50.240/32'
...
define cobbler_node($node_type, $mac, $ip, $power_id = undef) {
  cobbler::node { $name:
    mac 	   => $mac,
    ip 		   => $ip,
    ### UCS CIMC Details ###
...
# OpenStack controller. Change the ip to the IP address of your OpenStack controller
  ...
  cobbler_node { "control": node_type => "control", mac => "00:25:B5:00:05:0F", ip => "192.168.50.10", power_id  => "Folsom2" }
...
# Begin compute node
  cobbler_node { "compute1": node_type => "compute", mac => "00:25:B5:00:05:DE", ip => "192.168.50.21", power_id  => "Folsom3" }
  cobbler_node { "compute2": node_type => "compute", mac => "00:25:B5:00:05:BE", ip => "192.168.50.22", power_id  => "Folsom4" }
...
#  cobbler_node { "compute-server01": node_type => "compute", mac => "11:22:33:44:55:66:77", ip => "192.168.50.21", power_address  => "192.168.50.121:org-cisco", power_id => "OpenStack-1" }
# End compute node
...
node build-os-50 inherits build-node { }

# Change control_server to the host name of your control node
node control inherits os_base { class { control: crosstalk_ip => '192.168.50.10'} }

# Change compute_server01 to the host name of your first compute node
node compute1 inherits os_base { class { compute: internal_ip => '192.168.50.21', crosstalk_ip => '192.168.50.21'} }
node compute2 inherits os_base { class { compute: internal_ip => '192.168.50.22', crosstalk_ip => '192.168.50.22'} }
